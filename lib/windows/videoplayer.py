...
class VideoPlayerWindow(kodigui.ControlledWindow, windowutils.UtilMixin):
    ...
    def postPlay(self, video=None, playlist=None, handler=None, **kwargs):
        ...
        self.setProperty(
            'thumb.fallback', 'script.rrmj.extended/thumb_fallbacks/{0}.png'.format(self.prev.type in ('show', 'season', 'episode') and 'show' or 'movie')
        )
    ...
    def fillOnDeck(self):
        ...
        for ondeck in onDeckHub.items:
            ...
            if mli:
                ...
                mli.setProperty(
                    'thumb.fallback', 'script.rrmj.extended/thumb_fallbacks/{0}.png'.format(ondeck.type in ('show', 'season', 'episode') and 'show' or 'movie')
                )
