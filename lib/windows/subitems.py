...
class ShowWindow(kodigui.ControlledWindow, windowutils.UtilMixin):
    ...
    def updateProperties(self):
        ...
        if self.mediaItem.ratingImage:
            ...
            self.setProperty('rating.image', 'script.rrmj.extended/ratings/{0}.png'.format(self.mediaItem.ratingImage.replace('://', '/')))
            if self.mediaItem.audienceRatingImage:
                self.setProperty('rating2', audienceRating)
                self.setProperty('rating2.image', 'script.rrmj.extended/ratings/{0}.png'.format(self.mediaItem.audienceRatingImage.replace('://', '/')))
        else:
    ...
    def infoButtonClicked(self):
        fallback = 'script.rrmj.extended/thumb_fallbacks/{0}.png'.format(self.mediaItem.type == 'show' and 'show' or 'music')
    ...
    def fillExtras(self):
        ...
        for extra in self.mediaItem.extras():
            ...
            if mli:
                mli.setProperty('index', str(idx))
                mli.setProperty(
                    'thumb.fallback', 'script.rrmj.extended/thumb_fallbacks/{0}.png'.format(extra.type in ('show', 'season', 'episode') and 'show' or 'movie')
                )
                items.append(mli)
                idx += 1
    ...
    def fillRelated(self, has_prev=False):
        ...
        for rel in self.mediaItem.related()[0].items:
            ...
            if mli:
                mli.setProperty('thumb.fallback', 'script.rrmj.extended/thumb_fallbacks/{0}.png'.format(rel.type in ('show', 'season', 'episode') and 'show' or 'movie'))
                mli.setProperty('index', str(idx))
                items.append(mli)
                idx += 1
