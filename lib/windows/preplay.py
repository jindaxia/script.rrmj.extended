...
class PrePlayWindow(kodigui.ControlledWindow, windowutils.UtilMixin):
    ...
    def infoButtonClicked(self):
        opener.handleOpen(
            ...
            thumb_fallback='script.rrmj.extended/thumb_fallbacks/{0}.png'.format(self.video.type == 'episode' and 'show' or 'movie'),
    ...
    def setInfo(self):
        ...
        if self.video.ratingImage:
            ...
            self.setProperty('rating.image', 'script.rrmj.extended/ratings/{0}.png'.format(self.video.ratingImage.replace('://', '/')))
            if self.video.audienceRatingImage:
                self.setProperty('rating2', audienceRating)
                self.setProperty('rating2.image', 'script.rrmj.extended/ratings/{0}.png'.format(self.video.audienceRatingImage.replace('://', '/')))
        else:
