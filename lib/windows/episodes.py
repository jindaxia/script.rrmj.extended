...
class EpisodesWindow(kodigui.ControlledWindow, windowutils.UtilMixin):
    ...
    def infoButtonClicked(self):
        ...
        opener.handleOpen(
            ...
            sub_title=subtitle,
            thumb=episode.thumb,
            thumb_fallback='script.rrmj.extended/thumb_fallbacks/show.png',
    ...
    def setItemInfo(self, video, mli):
        ...
        if video.get('ratingImage'):
            ...
            mli.setProperty('rating.image', 'script.rrmj.extended/ratings/{0}.png'.format(video.ratingImage.replace('://', '/')))
            if video.get('audienceRatingImage'):
                mli.setProperty('rating2', audienceRating)
                mli.setProperty('rating2.image', 'script.rrmj.extended/ratings/{0}.png'.format(video.audienceRatingImage.replace('://', '/')))
        else:
    ...
    def fillEpisodes(self, update=False):
        ...
        self.episodeListControl.addItems(
            [kodigui.ManagedListItem(properties={'thumb.fallback': 'script.rrmj.extended/thumb_fallbacks/show.png'}) for x in range(len(episodes))]
        )
