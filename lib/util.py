...
def LOG(msg, level=xbmc.LOGNOTICE):
    xbmc.log('script.rrmj.extended: {0}'.format(msg), level)
...
def ERROR(txt='', hide_tb=False, notify=False):
    ...
    if hide_tb:
        xbmc.log('script.rrmj.extended: ERROR: {0} - {1}'.format(txt, short), xbmc.LOGERROR)
        return short

    import traceback
    tb = traceback.format_exc()
    xbmc.log("_________________________________________________________________________________", xbmc.LOGERROR)
    xbmc.log('script.rrmj.extended: ERROR: ' + txt, xbmc.LOGERROR)
...
def setGlobalProperty(key, val):
    xbmcgui.Window(10000).setProperty('script.rrmj.extended.{0}'.format(key), val)


def setGlobalBoolProperty(key, boolean):
    xbmcgui.Window(10000).setProperty('script.rrmj.extended.{0}'.format(key), boolean and '1' or '')


def getGlobalProperty(key):
    return xbmc.getInfoLabel('Window(10000).Property(script.rrmj.extended.{0})'.format(key))
...
def getProgressImage(obj):
    ...
    return 'script.rrmj.extended/progress/{0}.png'.format(pct)
